...
public class GeometricModels {
    ...
    private double modelCurranAsian(double S, double K, double r, double div, double sig, double T) {
        ...
        for (int i = 1; i <= n; i++) {
            summ2 = summ2 + (1 / n) * Math.exp(mui[i] + (vxi[i] / (Math.pow(vx, 2))) * (Math.log(K) - mu) + 0.5 * (Math.pow(vx, 2) + (Math.pow(vxi[i], 2) / Math.pow(vx, 2))));
        }
    ...
    private double thetaGeomethric(double S, double K, double T, double r, double div, double sig) {
        ...
        thetaGr = -(S * Math.exp((ba - r) * T) * ND(d1) * va) / (2 * Math.sqrt(T)) - ((ba - r) * S * CND(d1) * Math.exp((ba - r) * T)) - (r * K * Math.exp(-r * T) * CND(d2));
